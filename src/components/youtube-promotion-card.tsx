...
import type { YouTubePromotion } from "@/lib/sheba/types";
import { useYtPlayer } from "@/lib/sheba/yt-player";
import { recordClick, recordImpression, reportPromotion, toggleSavePromotion } from "@/lib/sheba/promotions";
    ...
    <article className={cn("group min-w-0", compact && "flex gap-3")}>
      ...
      <div className={cn(compact ? "min-w-0 flex-1" : "mt-2.5")}>
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">
          YouTube · promoted by{" "}
          <Link to="/artist/$slug" params={{ slug: promo.shebaArtistSlug }} className="underline">
            {promo.shebaArtistName}
          </Link>
        </p>
        <div className="mt-2 flex flex-wrap gap-1">
          ...
          <Link
            to="/artist/$slug"
            params={{ slug: promo.shebaArtistSlug }}
